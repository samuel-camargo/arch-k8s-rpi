...
# --- CONFIGURATION (Change these for each Worker) ---
HOSTNAME="kube-worker-01"
STATIC_IP="192.168.1.151/24"
...
docker run --rm --privileged \
    ...
    task 'Repartitioning'
    wipefs -af \"\$WORKER_IMG\" > /dev/null 2>&1
    fdisk \"\$WORKER_IMG\" <<EOF > /dev/null 2>&1
o
n
p
1
...
EOF
    ...
    wget -q \"http://fl.us.mirror.archlinuxarm.org/aarch64/core/\$KERNEL_PKG\"
    wget -q \"http://fl.us.mirror.archlinuxarm.org/aarch64/alarm/\$FIRM_RPI\"
    wget -q \"http://fl.us.mirror.archlinuxarm.org/aarch64/alarm/\$BOOT_PKG\"
    wget -q \"http://fl.us.mirror.archlinuxarm.org/aarch64/core/\$REGDB_PKG\"
