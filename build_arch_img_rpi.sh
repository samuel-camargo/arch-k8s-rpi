...
docker run --rm --privileged \
    ...
    task 'Injecting Advanced First-Boot Provisioning'
    cat <<'EOF_PROVISION' > /mnt/target/usr/local/bin/rpi-provision.sh
#!/bin/bash
# 1. Immediate WiFi Power Management Fix
iw dev wlan0 set power_save off || true

# 2. Wait for Internet connectivity (Lisbon/PT latency accounted for)
echo \"[PROVISION] Waiting for Google Ping...\"
while ! ping -c 1 -W 2 8.8.8.8 &>/dev/null; do
    sleep 3
done
echo \"[PROVISION] Internet detected. Starting Keyring setup...\"

# 3. Pacman Keyring Initialization
pacman-key --init
...
EOF
    ...
    task 'Patching Drivers & WiFi for PT'
...
EOF
    ...
    echo \"options cfg80211 regdomain=\$REG_DOMAIN\" >> /mnt/target/etc/modprobe.d/brcmfmac.conf
